<template>
    <div class="alrecord t_left">
        <el-dialog :title="dialogList | filterTitle" :visible.sync="dialogSync" width="1200px" top="2vh">   <!--:before-close="beforeClose"-->
            <DiaCredit :dialogList="dialogList" :gameType="dialogList.type" :baseNumber="dialogList.deskNumber" :chipData="dialogList.chipData" :realChipData="dialogList.realChipData" :ChipNumsList="allConst.ChipNumsList" :ChipTypeMap="allConst.ChipTypeMap" :curChosen="dialogList.currency ? dialogList.currency.id : ''" :CurrencyTypeMap="allConst.CurrencyTypeMap" @searchCli="searchCli" @closeDia="dialogSync = false"></DiaCredit>
        </el-dialog>
        <div class="titleBox">
...
<script>
import DiaCredit from "../boxs/common/DiaCredit"
export default {
    props:[],
    components:{
        DiaCredit
    ...
    methods:{
        ...
        // },
        searchCli(){
            this.$http.post(this.$Api.accountDeskCheckLogGetListApi,this.formData).then(
                (res)=>{
                    console.log("账房清点",res.data.data)
        ...
        audits(){
            var params = {deskCheckLogId:this.dialogList.id,status:"1",handlerInfo:this.handlerInfo};
